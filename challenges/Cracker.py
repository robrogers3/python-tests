...
class Puzzle:
    ...
    def __init__(self, board: Iterable[Iterable[bool]] = None, moves: Iterable[Move] = None):
        ...
        if moves is None:
            ...
            self.moves = Puzzle.all_moves(self)
        else:
            self.moves = moves
    ...
    def hop(self, move: Move) -> bool:
        if  not self.get(move, move.direction) or self.get(move, move.direction * 2) or not self.get(move, Puzzle.zero):
            return False
        self.set(move, Puzzle.zero, False)
        self.set(move, move.direction, False)
        self.set(move, move.direction * 2, True)
        return True
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Solve the Cracker Barrel Peg Game',
        epilog='Valid values of the puzzle_name and corresponding puzzle image follow:\n' +
        '\n'.join((puzzle_name + ':\n' + str(puzzle)
                for puzzle_name, puzzle in Puzzle.starting_puzzles.items())),
        formatter_class=argparse.RawDescriptionHelpFormatter)
